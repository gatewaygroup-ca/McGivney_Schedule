...
function initAuthGate() {
  ...
  auth.onAuthStateChanged((user) => {
    if (!user) { window.location.href = "https://gatewaygroup-ca.github.io/43-Munn-Schedule/login.html"; return; }
    ...
    db.ref("roles/" + user.uid).once("value").then((snap) => {
      ...
      if (!role) {
        ...
        document.getElementById("btnGateLogout").onclick = () => auth.signOut().then(() => window.location.href = "https://gatewaygroup-ca.github.io/43-Munn-Schedule/login.html");
        return;
      }
      STATE.userRole = role;
      ...
      document.getElementById("btnLogout").onclick = () => auth.signOut().then(() => window.location.href = "https://gatewaygroup-ca.github.io/43-Munn-Schedule/login.html");
